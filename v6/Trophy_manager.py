...
def sort_results(df):
    ...
    if sort_by:
        sort_col = 'product name' if sort_by == 'product_name' else ('code' if sort_by == 'code' else None)
    ...
    if key=="top":
        ...
        with col_sorting:
            ...
            with title:
                st.write("**Sort by:**")
            with sort_col1:
                arrow = "↑" if st.session_state['sort_by'] == 'product_name' and st.session_state['sort_order'] == 'asc' else ("↓" if st.session_state['sort_by'] == 'product_name' else "")
                ...
                if st.button(name_label, key=f"sort_name_{key}"):
                    ...
                    st.rerun()
            with sort_col2:
                arrow = "↑" if st.session_state['sort_by'] == 'code' and st.session_state['sort_order'] == 'asc' else ("↓" if st.session_state['sort_by'] == 'code' else "")
                ...
                with st.container():
                    ...
                    with col3:
                        with st.popover("Edit"):
                            with st.form(key=f"edit_form_{row['code']}_{idx}"):
                                name = st.text_input("Enter a new model name", key=f"namechange_{row['code']}_{idx}")
                                sport = st.text_input("Enter a new sport/category", key=f"sportchange_{row['code']}_{idx}")
                                submitted_edit = st.form_submit_button("Confirm")
                                if submitted_edit:
